...
Which comparisons do I want to make?
...
comparisons <- c(
  unlist(
    lapply(seq_along(ordered_stages),
           function(i){ 
             if(i < length(ordered_stages)){ 
               paste0(ordered_stages[i+1],'-',ordered_stages[i]) 
             } else {
               #When at the end, want it to cycle back round
               paste0(ordered_stages[1],'-',ordered_stages[i]) 
             } 
           }
    )
  )
...
stagesTrans_ht.mat <- filter(all_comparisons.df, FDR < 0.05) %>% 
  mutate(pathway.name = factor(pathway.name, 
                           levels = ordering(pathway.name, 
                                             order_vector = c('pirs', 'L', 'S', 'AAPL', 'ChAPL')))) %>% 
  arrange(pathway.name) %>% 
...
ht <- Heatmap(stagesTrans_ht.mat,
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        cluster_column_slices = FALSE,
        na_col = 'white',
        column_split = factor(str_extract(colnames(stagesTrans_ht.mat),
                                   pattern = 'pirs|^L|^S|AAPL|ChAPL'),
                              levels = unique(str_extract(colnames(stagesTrans_ht.mat),
                                   pattern = 'pirs|^L|^S|AAPL|ChAPL'))),
        column_order = colnames(stagesTrans_ht.mat),
