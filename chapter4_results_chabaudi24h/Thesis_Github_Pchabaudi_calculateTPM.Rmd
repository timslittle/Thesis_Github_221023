...
all.equal(apply(twentyfour_ct_comb[-1], 2, sum), 
          rep(1e6, times = dim(twentyfour_ct_comb[-1])[2]), 
          check.names = FALSE)
```
...
twentyfour_tpm_nopseudo <- chab_tpm.df %>% 
...
chab_tpm.df <- twentyfour_tpm_rescale_nopseudo
```

```{r clean_up_and_average}
# Change the names to remove superfluous 'rMT' 

chab_tpm.df <- rename_with(chab_tpm.df, 
                                  ~ str_extract(.x,pattern = '(?<=rMT_).+'),
                                  matches('rMT'))

twentyfour_rMT_avg <- melt(chab_tpm.df, 
