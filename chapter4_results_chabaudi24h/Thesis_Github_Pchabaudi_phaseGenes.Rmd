...
lapply(names(GOres.list), 
       function(GOres.num){
         ...
         #Get first (most significant) enriched GO id 
         goID <- GOres$gentable[1, "GO.ID"]
         goTerm <- GOres$gentable[1, "Term"]
         ...
         draw_heatmap_zscore(zscore_list = chab_z.list,
                             ...
                             show_row_names = FALSE,
                             tpm_mat = TRUE,
                             column_title_name = paste0(GOres.num, ' - ', goTerm)
         )
       })
...
Also these phaseograms show every gene in entire dataset associated with the top GO term, so they may not all be as out-of-phase as one another. This is a good test to see if this GO enrichment seems correct - if most of the genes look like they're not in the phase expected then the GO enrichment is probably erroneous.
