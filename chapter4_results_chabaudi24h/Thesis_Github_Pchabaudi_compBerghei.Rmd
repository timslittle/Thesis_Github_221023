...
ggplot(filter(chab_tpm_subfam.df.long, SorL != 'ancestral'), 
       aes(x = factor(name,
  ...
  scale_fill_manual(values = clade_col_num) +
...
ggplot(filter(berg_tpm_subfam.df.long, subfam != 'ancestral (n = 1)'), 
       aes(x = factor(name,
                      levels = ordering(name,
  ...
  geom_col(position = "fill") +
  xlab('stages') +
  scale_fill_manual(values = bir_clade_col_num) +
