...
```{r}
#Load for the showSigOfNodes network function.
library('Rgraphviz')

#Make a topGOdata object 
GOdata <- new("topGOdata", 
              ontology = "BP", 
              geneSel = function(x){x == 1},
              allGenes = z_max_diff.list$`1`,
              nodeSize = 5,
...
#Get first (most significant) enriched GO id 
goID <- goRes[4, "GO.ID"]
...
#Unlist the lapply below which goes through each element of the list and 
# returns TRUE is the go_id is listed for that gene.
#This makes a TRUE/FALSE index for the list/names of the list which can get 
# the genes associated with that GO id.
goID_genes <- names(go_annotation)[unlist(
  lapply(go_annotation, 
         function(go_ids){
           any(goID %in% go_ids)
         })
)]
...
draw_heatmap_zscore(zscore_list = chab_z.list,
                    filter_genes_vector = names(z_max_diff.list$`1`[z_max_diff.list$`2` == 1]),
                    gene_ordering_vector = phaseogram_time_order$rMT$zscore_order$Geneid,
                    threshold_tpm = 1,
                    use_times = TRUE,
                    use_order_vector = FALSE,
...
* 1\*3h Out-of-phase: Translation and peptide biosynthesis, gene expression and ribosome biogenesis.
* 2\*3h Out-of-phase: (Based on much fewer genes) Metabolism of carbohydrates and respiration.
* 3\*3h Out-of-phase: (Based on even fewerer genes) More metabolism and lipid biosynthesis.
* 4\*3h Out-of-phase: (Based on a few more genes actually) mRNA processing.

Fore node visualisation:
"Rectangles indicate the 5 most signicant terms. Rectangle color represents the relative signicance,
ranging from dark red (most signicant) to bright yellow (least signicant). For each node, some basic information
is displayed"
