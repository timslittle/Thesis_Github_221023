---
title: "tl22-01 BLASTing the HMMer matches from Plasmodium_Vir (using ORFs from getorf -find 1 option) to Plasmodium vivax P01 v56 proteins"
output: 
  html_document:
    theme: cosmo
    code_download: true
    ...
    toc_float:
      ...
      smooth_scroll: true
params:
  blast_output_file: '/camp/lab/langhornej/working/HPC/littlet/tl22-01_vivaxDeNovo/blast/blast_to_Proteins/scspades_getorfTableZero/SRX697844_SRR1571714/SRX697844_SRR1571714_scspades_getorfTableZero_hmmerPfam-Plasmodium_Vir_bast-VvxProt.outfmt6'
---

# Intro

This script is looks at the BLAST results of the HMMer matches with the Plasmodium_Vir among the ORFs from the Plasmodium vivax RNAseq scspades assembly. BLASTed to the Plasmodium vivax P01 PlasmoDB v56 proteins.
...
blast_all <- read.table(params$blast_output_file,
...
blast <- filter(blast_all, 
                sseqid %in% pir.info$Gene.ID) %>%
  group_by(qseqid) %>% 
  mutate(num_genes_match = n_distinct(sseqid))

#Non-pir genes
blast_nonpir <- filter(blast_all, 
                       !sseqid %in% pir.info$Gene.ID) %>%
  group_by(qseqid) %>% 
  mutate(num_genes_match = n_distinct(sseqid))
...
plot(y = unlist(transcripts_covered_by_denovo_diffcov), 
     ...
     main = 'For pir de novo transcripts',
