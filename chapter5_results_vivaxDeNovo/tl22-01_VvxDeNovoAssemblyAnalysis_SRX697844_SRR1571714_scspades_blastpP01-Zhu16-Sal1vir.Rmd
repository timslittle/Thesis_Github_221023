---
title: "tl22-01_VivaxPirsInTheDeNovoAssemblies SRX697844_SRR1571714_getorfFind1_blastp-P01-Zhu16_Sal1vir"
output: 
  html_document:
    theme: cosmo
    code_download: true
    ...
    toc_float:
      ...
      smooth_scroll: true
params:
  blast_output_file: '/camp/lab/langhornej/working/HPC/littlet/tl22-01_vivaxDeNovo/blast/blast_to_Proteins/scspades_blastpP01-Zhu16-Sal1vir/SRX697844_SRR1571714/SRX697844_SRR1571714_scspades_blastpP01-Zhu16-Sal1vir_hmmerPfam-Plasmodium_Vir_blastp-P01-all_Sal1-vir_Zhu2016Vir.outfmt6'
---

# Intro

This script is looks at the BLAST results of the HMMer matches with the Plasmodium_Vir among the ORFs from the Plasmodium vivax RNAseq scspades assembly. BLASTed to the Plasmodium vivax P01 PlasmoDB v56 proteins, the Zhu 2016 de novo mRNA vir predicted getorfs, and the Sal1 virs.
...
```{r transcripts_coverage_topBLAST_Zhu16transcripts_graph}
blast %>% 
  group_by(qseqid) %>%
  mutate(num_match_all = n_distinct(sseqid)) %>% 
  mutate(qseqid = paste0(qseqid, '; n = ', num_match_all),
...
There are `r length(unique(transcripts_covered_by_denovo$sseqid))` known transcripts covered by >=95% of at least one individual _de novo_ transcript. Of these `r length(unique(str_subset(transcripts_covered_by_denovo$sseqid, pattern = 'comp')))` are predicted virs from Zhu et al, 2016 and `r length(unique(str_subset(transcripts_covered_by_denovo$sseqid, pattern = 'PVX')))` are from Sal1 genome. There is a mean of `r mean(transcripts_covered_by_denovo$mismatch)` mismatches and `r mean(transcripts_covered_by_denovo$gapopen)` gaps and a median of `r median(transcripts_covered_by_denovo$mismatch)` mismatches and `r median(transcripts_covered_by_denovo$gapopen)` gaps in these alignments (Note that there are probably some de novo transcripts that align >=95% to the same known transcript). Of these `r length(unique(transcripts_covered_by_denovo$sseqid[transcripts_covered_by_denovo$num_match_all == 1]))`
