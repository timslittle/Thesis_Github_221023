...
sig_pirs <- bind_rows(shrunk.list) %>% 
  mutate(subfam = pir_clade$clade[match(Geneid, pir_clade$Geneid)]) %>% 
...
pir_column_split <- gsub(pir_clade$clade[match(colnames(sig_pirs_cast), pir_clade$Geneid)],
                         pattern = 'ancestral',
                         replacement = 'a')
pir_column_split <- ifelse(pir_column_split %in% c('S6', 'L2'), 
                           paste0(pir_column_split, '\n'),
                           pir_column_split)
