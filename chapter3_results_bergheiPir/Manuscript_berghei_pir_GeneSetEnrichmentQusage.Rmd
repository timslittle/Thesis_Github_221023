...
lapply(maleVfemale, getElement, 'qstable')
```
...
(
  x <- ggplot(filter(berghei_comparison_stats_df,
                     FDR < 0.05), 
              aes(x = contrast, 
...
order_path.names <- ordering(unique(mero_asex_malegam_stats_df$pathway.name), 
                             order_vector = c('pir', 
                                              'L', 'L[[:digit:]]', 
                                              'S', 'S[[:digit:]]'))
