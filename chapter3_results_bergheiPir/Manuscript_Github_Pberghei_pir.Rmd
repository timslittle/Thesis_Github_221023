...
pca_object_loadings[rev(
  ...
  allgene.info$Description[match(rownames(highest_loadings),
  allgene.info$Geneid)]

loadings_scale <- 0.005

bp <- plot +
  geom_point(data = pca_object_loadings,
             aes(x = PC1*loadings_scale, y = PC2*loadings_scale),
             inherit.aes = FALSE, size = 0.75) +
  geom_label_repel(data = pca_object_loadings,
             aes(x = PC1*loadings_scale, y = PC2*loadings_scale,
                 label = Geneid),
...
ggsave('thesis_chapter3_figure_biplot.png',
       bp)
