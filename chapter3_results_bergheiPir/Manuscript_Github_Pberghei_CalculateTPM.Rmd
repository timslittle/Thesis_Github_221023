...
library('ggdendro') #Drawing ggplot dendrograms
...
all.equal(apply(berghei_tpm[-1], 2, sum), 
          rep(1e6, times = dim(berghei_tpm[-1])[2]), 
          check.names = FALSE)
```
...
all.equal(berghei_tpm[3345:3350,colnames(check_tpm)],check_tpm)
...
lapply(experiment_id, 
       function(experiment_name){
         ...
         if(ncol(tpm_data) > 4){ 
           #No point doing hclusts for any of the expts with under four samples
           dists = dist(t(tpm_data))
           par(cex=0.7)
           p <- ggdendrogram(hclust(dists)) +
             ggtitle(experiment_name)
           ggsave(filename = paste0('thesis_chapter3_figure_hclust_', 
                                    experiment_name, 
                                    '.png'),
                  plot = p,
                  path = 'plots')
...
colnames(berghei_tpm) <- col_stage
```

Need to look more closely as to what I should and shouldn't remove from the hclust. E03_Asex.Schiz_3 looks like a well clustering sample, some of the 54 and 60h Liver samples do not look very similar to one another.
...
## Saving the counts with new names/samples removed

```{r save_new_counts}
berghei_counts[,str_subset(colnames(berghei_counts), 'E06_Liv.6h')] <- NULL

col_stage <- colnames(berghei_counts)
...
colnames(berghei_counts) <- col_stage

berghei_counts$`E08_Liv.pre4h_1` <- NULL
berghei_counts$`E03_Asex.Schiz_1` <- NULL
berghei_counts$`E03_Asex.Schiz_3` <- NULL

write.csv(berghei_counts,
          'berghei_counts_newNames.csv')
```
